...
resource "aws_api_gateway_domain_name" "main" {
  certificate_arn = var.main_domain_wildcard_cert_validation_arn
  domain_name     = "${var.service_name}-api.${var.domain}"
}
    ...
    "Statement": [
        {
            ...
            "Resource": "${aws_api_gateway_rest_api.main.execution_arn}/*/POST${aws_api_gateway_resource.jenkins_github_webhook.path}",
            "Condition": {
                "IpAddress": {
...
resource "aws_api_gateway_authorizer" "github_webhook" {
  ...
  rest_api_id                      = aws_api_gateway_rest_api.main.id
  authorizer_uri                   = aws_lambda_function.github_webhook_authorizer_api_gw_authorizer.invoke_arn
...
resource "aws_api_gateway_api_key" "github_webhook" {
  name        = "${var.service_name}-github-webhook"
